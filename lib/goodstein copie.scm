...
;;
;; e-mail: damien.mattei@orange.fr 
...
;; definition:
;; at left is LisP/Scheme notation, at right is mathematical notation
;; polynomial : (+ e_n e_n-1 e_n-2 ... e_k) = Cn.b^n + Cn-1.b^(n-1) .... + Ck.b^k
;; note: e_n are monomials , e_n is a monomial of degree n, in the list monomial are sorted from higher to lower degree
;;
;;
;; -GET THE MONOMIAL OF LOWER DEGREE: reverse polynomial list to get e_k the first monomial of lower degree 
;;  (e_k .... e_n-2 e_n-1 e_n +) = Ck.b^k + ... + Cn-1.b^(n-1) + Cn.b^n
;;
;; -keep the rest :
;;  (e_k+1 .... e_n-2 e_n-1 e_n +) = Ck+1.b^(k+1) + ... + Cn-1.b^(n-1) + Cn.b^n
;;
;; -COMPUTE MONOMIAL - 1:
;;  compute Ck.b^k - 1 in hereditary base by calling hereditary-base-monomial-1 recursive function
(define (symbolic-polynomial-1 hs) ;; hs : hereditary successor polynomial, g = symbolic-polynomial-1

  (let ((monomial '())
	(monomial-1 '())
	(hs-rev '())  ;; hs reverse
	(hs-rev-rest '()) ;; hs reverse rest
	(h '())) ;; result h becomes the new hereditary polynomial 
    ...
    (cond
     ...
     ((and    ;; not number and addition of monomials
      ...
      ;;
      (set! hs-rev (reverse hs)) ;; reverse polynomial list to get e_k the first monomial of lower degree 

      ;; (e_k .... e_n-2 e_n-1 e_n +) = Ck.b^k + ... + Cn-1.b^(n-1) + Cn.b^n

      
      ;;(dv hs-rev)
      (set! monomial (first hs-rev)) ;; get lower degree monomial : Ck.b^k 
      ;;(dv monomial)
      ;;(display (prefix->infix (expt->^ monomial)))
      ;;(display " - 1 = ")
      ...
      (cond (
	     ...
	     ;; number
	     ;;
	     (number? monomial-1) ;; monomial-1 = number = m'
		 ...
		 (if (pair-list? hs-rev-rest) 
		     ;; then
		     ;; hs-rev-rest = ( e_n + ) = Cn.b^n + 0
		     (set! h (first hs-rev-rest)) ;; remove the no more usefull operator : h = e_n = Cn.b^n
		     ;; else
		     ;; hs-rev-rest = ( e_k+1.... e_n-2 e_n-1 e_n +) = Ck+1.b^(k+1) + ... + Cn-1.b^(n-1) + Cn.b^n
		     (set! h (reverse hs-rev-rest))) ;; h = (+ e_n e_n-1 e_n-2 ... e_k+1) = Ck+1.b^(k+1) + ... + Cn-1.b^(n-1) + Cn.b^n
		 
		 ;; not zero
		 (set! h ;; reconstruct the polynomial
		       (reverse
			(insert monomial-1 hs-rev-rest))))) ;; (monomial-1 e_k+1 .... e_n-2 e_n-1 e_n +)
	    ;; (+ e_n e_n-1  ....  e_k+1 m') = m' + Ck+1.b^(k+1) + ... + Cn-1.b^(n-1) + Cn.b^n

	    #|((is*? monomial-1) ;; multiplication : monomial-1 = c.b^s, ce cas semble ne jamais avoir lieu car P(b)-1 n'est jamais de cette forme  
	     (set! h
		   (reverse (insert monomial-1 hs-rev-rest))))|# ;; reconstruct the polynomial

	    (else ;; monomial-1 = m' = (+ e'_n' e'_n'-1 .... e'_k') , higher to lower monomial degrees
	     
	     (set! h
		   ;; (+ e_n e_n-1  ....  e_k+1 e'_n' e'_n'-1 .... e'_k') =  C'k.b^k' + ... + Cn'-1.b^(n'-1) + C'n.b^n' + Ck+1.b^(k+1) + ... + Cn-1.b^(n-1) + Cn.b^n
...
(define (hereditary-base-monomial-1 P) ;; f = hereditary-base-monomial-1
  ...
  ;; P is a monomial, P = c.b^n or P = b^n or P = b
  
  ;; (dv P)

  ;; f : P --> P - 1

  ;; propertie : f( P + Q ) = P + Q - 1 = P + f(Q)
  ...
  (cond ((number? P) ;; P = b -> f(P) = b-1
	...
	((is*? P) ;; P = (* c b^n) = c.b^n = (c-1).b^n + b^n -> f(P) = f( (c-1).b^n + b^n ) = (c-1).b^n + f(b^n)
	...
	(else ;; P = b^n = b.b^(n-1) = (b-1).b^(n-1) + b^(n-1) -> f(P) = f( (b-1).b^(n-1) + b^(n-1) )
	 ;;                                                            = (b-1).b^(n-1) + f(b^(n-1))
	 ;;  with n-1 computed with g(n) , g = symbolic-polynomial-1
...
(define (goodstein-init n)
  ...
  (let ((n-start n) ;; n at start
    ...
    (letrec ((goodstein-rec
	      ...
	      (lambda (h) ;; hereditary base b expression
